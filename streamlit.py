...
hangman_images = ['hangmanpics/1.jpg', 'hangmanpics/2.jpg', 'hangmanpics/3.jpg', 'hangmanpics/4.jpg', 'hangmanpics/5.jpg']

word_list = ['tuple', 'python', 'streamlit', 'programming', 'coding']  # List of words to choose from

while True:
    ans = choice(word_list)  # Choose a random word from the list
    word = "_ " * len(ans)  # Initialize word with underscores
    count = 1  # for counting the input
    letters = [''] * len(ans)  # for storing inputs

    display.markdown(f"<h1 style='text-align: left; color: cyan;'>{word}</h1>", unsafe_allow_html=True)

    chances = len(ans) + 4  # 4 wrong attempts are acceptable

    t5.image(hangman_images[0])

    for i in range(len(ans)):
        letters[i] = t1.text_input(f"Enter a letter:", max_chars=1, key=i)

    j = 0
    k = len(ans)
    while True:
        if j < len(ans) and letters[j] and count != chances and l_count != 4 and w_count != len(ans):
            win_check(letters[j], ans)
            word = update(letters[j], word, ans)
            j += 1
            k += 1
            count += 1
            if k < len(ans):
    ...
    if l_count == 1:
        t5.image(hangman_images[1])
    elif l_count == 2:
        t5.image(hangman_images[2])
    elif l_count == 3:
        t5.image(hangman_images[3])
    elif l_count == 4:
        t5.image(hangman_images[4])
